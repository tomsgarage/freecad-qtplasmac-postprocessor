...
real GCode suitable for a plasmac controlled cnc plasma cutter. This postprocessor, once placed
in the appropriate PathScripts folder, can be used directly from inside
FreeCAD, via the GUI importer or via python scripts with:

import plasmac_post
plasmac_post.export(object,"/path/to/file.ncc","")
'''
...
#<holes> = 3 (Plasmac reduces feedrate for holes < 1.26" and arcs < 0.63" to 60%.)
'''
...
def parse(pathobj):
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        for c in pathobj.Path.Commands:
            ...
            for param in params:
                if param in c.Parameters:
                    if param == 'F' and (currLocation[param] != c.Parameters[param] or OUTPUT_DOUBLES):
                        ...
                        #    if torchOn == False:
                                #out += linenumber() + "M03 $0 S1\n"
                                #torchOn = True
                        #elif speed.getValueAs(UNIT_SPEED_FORMAT) == 0.0:
                        #    if torchOn == True:
                                #out += linenumber() + "M05\n"
                                #torchOn = False
                        continue
                        ...
                        continue #outstring.append(param + str(int(c.Parameters['S'])))
                    elif param == 'Z':   # plasmac does not use Z moves from gcode
                        if 'X' in c.Parameters or 'Y' in c.Parameters:
                            continue # because Z is the last parm we're interested in.
            ...
            if command == 'G81':
                outstring = []
                out += linenumber() + "M3 $2 S1 (SPOTTING ON)\n"
                out += linenumber() + "G91\n"
                out += linenumber() + "G1 X0.000001\n"
                out += linenumber() + "G90\n"
                out += linenumber() + "M5 $2 (OFF)\n"
